...
const ChatBox: React.FC<ChatBoxProps> = ({ activeChat, messages }) => {
  ...
  return (
    <main
      className="flex-1 bg-[#F5F6FA] flex flex-col h-full w-[1070px]"
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center gap-4">
          <img src={`https://i.pravatar.cc/40?u=${activeChat}`} alt="Avatar" className="w-10 h-10 rounded-full object-cover" />
          <h2 className="font-bold text-xl text-gray-800">{activeChat}</h2>
        </div>
        <div className="flex items-center gap-3 text-white">
          <button className="h-10 w-10 bg-[#611DD0] rounded-full flex justify-center items-center"><FiPhoneCall size={20} /></button>
          <button className="h-10 w-10 bg-[#611DD0] rounded-full flex justify-center items-center"><FiVideo size={20} /></button>
          <button className="h-10 w-10 bg-[#611DD0] rounded-full flex justify-center items-center"><BsThreeDots size={24} /></button>
        </div>
      </div>

      {/* Messages */}
      <div ref={scrollRef} 
      style={{ backgroundImage: `url(${backgroundChatImage})`, backgroundSize: "cover", backgroundPosition: "center" }}

      className="flex-1 p-4 overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-purple-500/50 scrollbar-track-gray-100">
        {messages.filter((msg) => msg.chatUser === activeChat).map((msg) => (
          <div key={msg._id} className={`flex ${isMe(msg) ? "justify-end" : "justify-start"}`}>
            <div className={`rounded-2xl p-3 max-w-xs ${isMe(msg) ? "bg-purple-600 text-white" : "bg-gray-200 text-gray-800"}`}>
              {msg.messageType === "text" && <p>{msg.content}</p>}
              {msg.messageType === "image" && msg.mediaUrl && <img src={msg.mediaUrl} alt="image" className="w-110 h-70 rounded-lg" />}
              ...
              <div className="text-xs opacity-70 mt-1">{formatTimestamp(msg.createdAt)} - {msg.status}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="p-4 flex absolute w-[1110px] rounded-2xl bottom-0 items-center gap-3 border-t border-gray-200 bg-white">
        ...
        <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center bg-gray-200 text-gray-700 rounded-full h-10 w-10 hover:bg-gray-300 transition">
