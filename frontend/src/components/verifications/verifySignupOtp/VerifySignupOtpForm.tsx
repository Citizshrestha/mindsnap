...
const VerifySignupOtpForm = () => {
  ...
  useEffect(() => {
    ...
    if (!registrationSuccess && (!userId || !email)) {
  ...
  }, [userId, email, navigate, registrationSuccess]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await verifySignupOtp(userId, formData.otp);
      
      console.log("✅ OTP verification successful:", response);
      
      // Set success flag to prevent useEffect from redirecting
      setRegistrationSuccess(true);
      
      toast.success(response.message || "User registration successful! You can now log in. 🎉");
      ...
      console.log("🧹 localStorage cleaned up, navigating to login...");
      
      // Navigate to login page after a short delay to ensure state is updated
      setTimeout(() => navigate("/"), 100);
    } catch (err: unknown) {
      let errorMessage = "An unexpected error occurred. Please try again.";
      let shouldNavigateToVerifyEmail = false;
      ...
      // Navigate to verify email if session expired
      if (shouldNavigateToVerifyEmail) {
        console.log("🔄 Navigating to verify email due to session expiry");
...
export default VerifySignupOtpForm;
